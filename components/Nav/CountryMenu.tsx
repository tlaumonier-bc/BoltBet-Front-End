...
export default function CountryMenu({ countries }: { countries: Item[] }) {
  ...
  // Tap-to-open needs an explicit close: outside click or Escape.
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div
      ref={ref}
      className="relative"
      // Hover-open only for mouse pointers, so a touch tap doesn't open-then-close.
      onPointerEnter={(e) => {
    ...
    >
      ...
      {/* Panel always rendered; only visibility toggles (keeps links crawlable). */}
      <div
        className={`absolute right-0 top-full mt-2 w-[min(520px,86vw)] transition ${
          open
            ? 'visible translate-y-0 opacity-100'
            : 'invisible -translate-y-1 opacity-0'
        }`}
      >
        <div className="glass max-h-[60vh] overflow-y-auto rounded-2xl p-2">
